...
alter table public.solicitacoes_acesso
  drop constraint if exists solicitacoes_acesso_status_v90_check;
alter table public.solicitacoes_acesso
  add constraint solicitacoes_acesso_status_v90_check
  check (status in ('pendente','aprovado','rejeitado'));

alter table public.solicitacoes_acesso
  drop constraint if exists solicitacoes_acesso_perfil_v90_check;
alter table public.solicitacoes_acesso
  add constraint solicitacoes_acesso_perfil_v90_check
...
create policy "solicitacoes_acesso_insert_public_v90"
on public.solicitacoes_acesso
for insert
to anon, authenticated
with check (
  status = 'pendente'
  and perfil_atribuido is null
...
create policy "solicitacoes_acesso_admin_select_v90"
on public.solicitacoes_acesso
for select
to authenticated
using (
  exists (
    ...
    where p.id = (select auth.uid())
      and p.ativo is true
      and (
        p.role in ('master','pastor','secretaria')
        or lower(coalesce(p.cargo,'')) like '%secretar%'
      )
  )
);

-- As decisões são realizadas exclusivamente pela Edge Function com service role.
-- Nenhum utilizador do navegador recebe UPDATE direto nesta tabela.
